class NexusUltraV4 {
    ...
    tokenize(code) {
        ...
        const tokensDef = [
            ['COMMENT', /#.*/],
            ['KEYWORD', /\b(fn|set|out|if|else|loop|while|for|end|wait|input|return|break|import)\b/],
            ['NUMBER', /\d+(\.\d+)?/],
            ['STRING', /"[^"]*"/],
            ['MODULE_FUNC', new RegExp(`\\b(${modKeys})\\.[a-zA-Z_]\\w*`)],
            ['ID', /[a-zA-Z_]\w*/],
            ['OP', /[+\-*/><=!&|]+/],
            ['LPAREN', /\(/], ['RPAREN', /\)/], ['COMMA', /,/],
            ['ASSIGN', /=/], ['SKIP', /[ \t\n\r]+/], ['MISMATCH', /./]
    ...
    async run(tokens) {
        ...
        while (i < tokens.length) {
            ...
            } else if (type === 'MODULE_FUNC') {
                let [res, nextI] = await this.parseModuleCall(tokens, i);
                i = nextI;
